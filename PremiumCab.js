...
fetch('https://aimcabbooking.com/get_pincode_api.php', {
  ...
  if (roundfare) {
    ...
    roundfarePrice = roundfareObject.sedan; // Store the fare price in the variable
  }
  ...
  if (fare) {
    // Check if the fare state is available
    const fareObject = fare[0]; // Assuming the fare data is an array with a single object
    farePrice2 = fareObject.SedanPremium; // Store the fare price in the variable
  }
  ...
  if (roundfare) {
    // Check if the fare state is available
    const roundfareObject = roundfare[0]; // Assuming the fare data is an array with a single object
    roundfarePrice2 = roundfareObject.SedanPremium; // Store the fare price in the variable
  }
      ...
      >
        <View style={styles.cardsContainer}>
         ...
         {/* <View style={styles.cardsContainer1}>
          {data
             .filter((item) =>  item.model_type === "SEDANPREMIUM")
            .map((item) => (
              <Card
                key={item.id}
                data={item}
